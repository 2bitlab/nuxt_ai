...
const getDefConfig = () => {
  const config = {
    ...
    allowActions: [
      ...
      'name/ci:DescribeMediaJob',
      'name/ci:CreateAsrJobs',
    ...
    statement: [
      {
        ...
        resource: [
          ...
          `qcs::ci:${config.region}:uid/${appId}:bucket/${config.bucket}/${config.allowPrefix}`,
          // '*',
        ],
...
export const getCredentialAuth = async () => {
